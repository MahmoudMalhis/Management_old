...
import { useAsyncState, useErrorHandler } from "@/hooks/useErrorHandler";
...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      setLoading(true);
      // جلب الإنجازات
      const accomplishmentsRes = await accomplishmentsAPI.getAccomplishments();

      // استخراج المصفوفة بشكل صحيح
      const allAccomplishments = Array.isArray(accomplishmentsRes)
        ? accomplishmentsRes
        : accomplishmentsRes?.data || [];

      // تصفية حسب الفترة
      const filteredAccomplishments = filterByPeriod(
        allAccomplishments,
        period
      );

      // حساب عدد المراجعات المعلقة
      const pendingReviews = allAccomplishments.filter(
      ...
      setStats({
        totalEmployees,
        pendingReviews,
        recentAccomplishments: filteredAccomplishments,
      });
    } catch (err) {
  ...
  return (
    <div className="space-y-6 min-h-screen p-5 bg-gradient-to-br from-[#daeaff] via-[#f5f8fa] to-[#c9e4ff]">
      ...
      {error && <ErrorCard error={error} onRetry={fetchDashboardData} />}
      {!loading && !error && (
        <>
          <div>
